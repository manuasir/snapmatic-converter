...
class SnapConverter {
  ...
  constructor(srcPath = `${baseDir}/source`, dstPath = `${baseDir}/converted`) {
    ...
    this._srcPath = this.path.normalize(srcPath);
    this._dstPath = this.path.normalize(dstPath);
  }
  ...
  convertSingleFile(fileName) {
    try {
      ...
      const mergedPath = this.path.join(this.srcPath,file[0])
      const convertedImg = this.convert(
        this.fileToBuffer(mergedPath)
      );
      this.writeFile(this.path.join(this.dstPath,file[0]), convertedImg);
      debug &&
        console.log(
  ...
  convertAllFiles() {
    try {
      ...
      for (const file of files) {
        const convertedImg = this.convert(
          this.fileToBuffer(this.path.join(this.srcPath,file))
        );
        this.writeFile(this.path.join(this.srcPath,file), convertedImg);
        debug &&
          console.log(
            `Successfully converted the ${this.srcPath}/${file} image in ${
              this.dstPath
